defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    test "the next question is randomly select", %{quiz: quiz} do
  ...
  defp quiz(context) do
    {:ok, Map.put(context, :quiz, QuizBuilders.build_quiz_with_two_templates())}
  end

  defp quiz_always_adds_one_and_two(context) do
    fields = template_fields(generators: QuizBuilders.addition_generators([1], [2]))
